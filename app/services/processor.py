...
async def file_processor(
    job_id: str, file_path: str, correlation_id: str, db_session=None
):
    """
    ...
    try:
        ...
        aggregate_data = {
            ...
            "average": (
                sum(float(row["value"]) for row in total if "value" in row) / len(total)
                if total
                else 0
            ),
        }
        ...
        await db_session.execute(
            ...
            .values(status=JobStatus.COMPLETED, updated_at=datetime.utcnow())
        )
        ...
        await db_session.execute(
            Job.__table__.update()
            .where(Job.correlation_id == correlation_id)
            .values(status=JobStatus.FAILED, updated_at=datetime.utcnow())
        )
        await db_session.commit()
